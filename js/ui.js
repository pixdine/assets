...
const sliderBanner = function () {
  const $slider = $('.swiper.banner-slider');
  ...
  $('.banner-slider .swiper-slide').on('mouseover', function () {
    swiper.autoplay.stop();
  });
  $('.banner-slider .swiper-slide').on('mouseleave', function () {
    swiper.autoplay.start();
  });
  $('.swiper.banner-slider .slider-button').on('click', function () {
    swiper.autoplay.start();
  });
};
const slideWebzin = function () {
  if($('.webzin-slider .swiper-slide').length > 1){
    $('.webzin-slider .controller').show();
    const $slider = $('.swiper.webzin-slider');
    const swiper = new Swiper('.swiper.webzin-slider', {
      speed: 400,
      loop: true,
      slidesPerView: 1,
      autoplay: {
...
const applytabletZoom = function() {
  ...
  if(pathname.endsWith('pages/award/mjmirae11.mrn')){
    return ;
  }
  if(zoomRatio < 1){
